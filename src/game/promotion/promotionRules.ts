...
export function getPromotionRecommendations(
  ...
  if (preview.readiness === "ready" || preview.readiness === "almost") {
    recommendations.push("A promoção real ainda não está disponível; use esta prévia para planejar o investimento.");
  }
...
// Placeholder explicito: nao altera estado nesta versao.
export function promoteHero(
  ...
  void state;
  void heroId;
  return { ok: false, message: "Promoção real ainda não está disponível nesta versão." };
}
