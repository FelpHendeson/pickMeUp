...
- `src/game/promotion/` administra a fundacao da ascensao por estrelas (somente leitura). `getHeroPromotionPreview` deriva elegibilidade, requisitos, readiness (`blocked`, `not-ready`, `almost`, `ready`), beneficios projetados, riscos e recomendacoes a partir de `hero.rarity`, `hero.level`, analise de potencial, proficiencias, tecnicas leves, moral, ferimentos e andar da Torre. Nao persiste estado, nao altera `hero.rarity`, `maxLevel`, stats, combate, summon, expedicoes nem consome materiais. `promoteHero` existe como placeholder bloqueado. Material de Eco aparece como requisito futuro (`warning`) na rota 4★→5★.
...
- O painel de Herois exibe a secao "Ascensao por Estrelas": raridade atual e alvo, status de readiness, requisitos com status (met/missing/warning), beneficios projetados, riscos, recomendacoes e aviso de que a promocao real ainda nao esta disponivel. O botao "Promover" fica desabilitado.
