...
- O progresso e salvo localmente no navegador com `localStorage`, `saveVersion: 1` e `schemaVersion: 4`. Saves legados passam por migrations sequenciais antes da normalizacao final; campos ausentes recebem defaults e referencias invalidas sao removidas. A migration para o schema 3 adiciona a estrutura de treino (`training`) e a migration para o schema 4 adiciona a estrutura de proficiencias (`proficiencies`), ambas com defaults seguros.
